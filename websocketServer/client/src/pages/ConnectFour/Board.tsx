      ...
      {board.map((row, i) => (
        <div key={i} className="Row w-full h-[16.66%] flex">
          {row.map((square, j) => (
            ...
            >
              {square === ConnectFour.Square.Red ? (
                <div className="w-[85%] aspect-square bg-red-500 rounded-full" />
              ) : square === ConnectFour.Square.Blue ? (
                <div className="w-[85%] aspect-square bg-blue-500 rounded-full" />
              ) : null}
            </div>
          ))}
        </div>
